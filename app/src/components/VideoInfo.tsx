...
export function VideoInfo({ item }: VideoInfoProps) {
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2 border-b border-zinc-800/50">
                    ...
                    {/* Toolbar (Only show in view mode or keep it? Standard actions like Like/Fav should probably stay accessible) */}
                    {/* Let's keep them accessible, but maybe disable them or hide them if they conflict visually? Nah, they are fine. */}
                    {!isEditing && (
                        <VideoActions
                            id={item.id}
            ...
            <div className={isEditing ? "bg-zinc-900/50 p-4 rounded-xl border border-zinc-800/50" : "bg-zinc-900/50 rounded-xl p-4 text-sm text-zinc-300 whitespace-pre-wrap hover:bg-zinc-900 transition-colors"}>
                ...
                {isEditing ? (
                    ...
                    <>
                        ...
                        {/* Display Tags in View Mode (Non-editable list usually, but since TagEditor is interactive, we could just show it or show readonly tags. 
                   User requested "edit functionality with description". 
                   So maybe in View Mode we just show tags as pills? 
                   Or keep TagEditor but maybe styled differently? 
                   Current TagEditor allows add/remove immediately. 
                   Maybe we should make TagEditor READONLY in view mode? 
                   Or just hide the input?
                   Let's show tags as static pills in view mode for cleaner read-only look, and full editor in Edit mode.
               */}
